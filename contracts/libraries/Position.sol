...
library Position {
    /// @notice Info stored for each user's position.
    struct Info {
    ...
    function update(
        ...
        if (liquidityDelta == 0) {
            require(_self.liquidity > 0, "Position: NP"); // disallow pokes for 0 liquidity positions
        ...
        unchecked {
            ...
            if (tokensOwed0 > 0 || tokensOwed1 > 0) {
                // Overflow is acceptable, user must withdraw before they hit
                // type(uint128).max fees.
